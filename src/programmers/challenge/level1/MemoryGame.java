 ...
 * description: 사진들을 보며 추억에 젖어 있던 루는 사진별로 추억 점수를 매길려고 합니다.
 ...
 * 이 사진의 추억 점수는 3명의 그리움 점수를 합한 67 (11 + 1 + 55)점 입니다.
 * 그리워하는 사람의 이름을 담은 문자열 배열 name, 각 사람별 그리움 점수를 담은 정수 배열 yearning,
 * 각 사진에 찍힌 인물의 이름을 담은 이차원 문자열 배열 photo가 주어질 때,
 * 사진들의 추억 점수를 photo에 주어진 순서대로 배열에 담아 return 하는 solution 함수를 완성해주세요.
 */
public class MemoryGame {
    public static void main(String[] args) {
        ...
        int[] answer = memoryGame.solution(name, yearning, photo);
    ...
    private int[] solution(String[] name, int[] yearning, String[][] photo) {
        ...
        for (int i = 0; i < photo.length; i++) {
            for (int j = 0; j < photo[i].length; j++) {
                result[i] += yearningMap.getOrDefault(photo[i][j], 0);
            }
        }
        return result;
    }
